...
export function OrderDeleteConfirm({ open, orderNumber, onClose, onConfirm, loading }: OrderDeleteConfirmProps) {
  if (!open) return null
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-sm animate-slide-up rounded-xl border border-white/5 bg-surface-900 p-5 shadow-2xl">
        <div className="flex flex-col items-center text-center">
          ...
          <h3 className="text-sm font-semibold text-gray-100">Siparisi Sil</h3>
          <p className="mt-1 text-xs text-gray-400">
            <span className="font-medium text-gray-300">{orderNumber}</span> numarali siparisi silmek istediginizden emin misiniz?
          </p>
        </div>
